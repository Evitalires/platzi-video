...
class Home extends Component {
  state = {
    ...
    mediumDevice: window.matchMedia("screen and (max-width: 767px)")
  ...
  handleSetMedia = (media) => {
    ...
    setTimeout(() =>  this.setState({ nuevoVideo: false }, 500));
  }
  ...
  componentDidMount  = () => {
    this.validationScreen(this.state.mediumDevice);
    this.state.mediumDevice.addListener(this.validationScreen); 
  }
  render () {
    return (
      <HandleError>
        <HomeLayout setRef={this.setRefHome} screenSize={this.state.screenSize}>
          <Related
            screenSize={this.state.screenSize}
            showRelatedBurger={this.state.showRelatedBurger}
